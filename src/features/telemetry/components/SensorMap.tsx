...
export default function SimpleSensorMap({ sensors }: { sensors: SensorData[] }) {
  ...
  return (
    <div
      ...
      className="h-[400px] w-full rounded-2xl overflow-hidden border border-muted/20 shadow-sm relative bg-background group/map"
    >
      
      {/* 1. Legend */}
      <div
        data-testid="sensor-legend"
        className="absolute top-4 left-4 z-10 bg-background/80 backdrop-blur-md p-3 rounded-xl border border-muted/20 shadow-md"
      >
          ...
          return (
            <Overlay key={sensor.sensorId} anchor={[lat, lon]} offset={[6, 6]}>
              <div
                ...
                data-testid="sensor-marker"
                data-sensor-id={sensor.sensorId}
                data-sensor-type={sensor.sensorType}
              >
                ...
                {/* Tooltip */}
                <div
                  data-testid="sensor-tooltip"
                  className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-max max-w-[150px] hidden group-hover:block bg-foreground text-background text-[10px] px-3 py-2 rounded-lg shadow-xl z-50 pointer-events-none"
                >
